...
**2026-06-16 — Rebranded to SMOALD Living, and it went live in the Store.** Renamed the shop from "SMOLD & Co." to **SMOALD Living**, moved it to its own `SMOALD-Living` repo, and linked the live demo into the SMOALD.com Store — so the hub now points to a real, shipped product. **Key lesson:** a brand name lives in more places than you expect (the page, the code comments, the docs, and other sites that link to it), so a rename only feels finished after a sweep across all of them.
